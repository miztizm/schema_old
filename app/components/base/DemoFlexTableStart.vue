...
const props = withDefaults(
  defineProps<{
    ...
    html?: boolean // New prop for rendering subtitle as raw HTML
  }>(),
  {
    subtitle: undefined,
    icon: undefined,
    ...
    html: false, // Default to false so it doesn't render as raw HTML by default
  },
)
const trimmedSubtitle = computed(() => {
  if (props.trimSubtitle && props.subtitle) {
    return props.subtitle.length > 55 ? `${props.subtitle.substring(0, 55)}...` : props.subtitle;
  }
  ...
  >
    ...
    <div>
      ...
      <BaseParagraph
        size="xs"
        lead="tight"
        class="text-muted-500 dark:!text-primary-600 md:!text-xs"
      >
           <!-- Conditionally render with v-html if html is true -->
           <span v-if="props.html" v-html="trimmedSubtitle"></span>
        <span v-else>{{ trimmedSubtitle }}</span>
      </BaseParagraph>
    </div>
  </div>
</template>
