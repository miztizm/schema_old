  ...
  <div :class="ui.wrapper" v-bind="attrs">
    ...
    <div :class="ui.container">
      <div v-if="providers?.length" :class="ui.providers">
        ...
        block
        @click="$emit('provider', provider.click)"
        />
      </div>
      ...
      >
        ...
        >
          <slot :name="`${field.name}-field`" v-bind="{ state, field: omit(field, ['description', 'help', 'hint', 'size']) }">
            ...
            <UInput v-else icon="i-ph:user-duotone" v-model="state[field.name]" v-bind="omit(field, ['label', 'description', 'help', 'hint', 'size'])" />
          </slot>
...
const props = defineProps({
  ...
  providers: {
    type: Array as PropType<Array<Button & { click: string }>>,
    default: () => []
  },
  submitButton: {
...
const emit = defineEmits(['submit', 'provider'])
...
const config = computed(() => {
  ...
  return {
    ...
    providers: 'space-y-3',
    form: 'space-y-6',
    footer: 'text-xs text-gray-500 dark:text-gray-400 mt-2',
    passwordToggle: {
